...
if pagina_seleccionada == 'Inicio':
    ...
    col1.image("yoporolo.jpg", caption='(yo con mi pequeña camarita)', width=320)
    ...
    texto = """
    Mi nombre es Sandra Yñiguez, tengo 19 años y nací en Lima, Perú. Actualmente me encuentro estudiando la carrera de Periodismo. Me interesan muchas cosas, pero especialmente la literatura (tengo una gran debilidad por los cuentos), el cine y todo lo que tenga que ver con la producción audiovisual. También paso buena parte del día escuchando música y explorando géneros nuevos, que para mí es casi tan esencial como escribir. De mi carrera, he desarrollado un interés particular por la fotografía periodística y, a futuro, me gustaría desarrollar mejor esa faceta.
    """
    ...
    texto_2 = """
    Acercarme a Python fue un desafío. Al tratarse de un lenguaje completamente nuevo para mí, al inicio me sentí un poco perdida, pero conforme fui explorando sus posibilidades, me sorprendió gratamente lo mucho que podía hacer con él. Descubrí que no solo tiene aplicaciones prácticas para el periodismo, como el análisis de datos o la automatización de tareas, sino que también puede integrarse en aspectos cotidianos de forma muy útil. Me dio una nueva forma de entender a la programación como aliada en la comunicación y la creatividad. Hoy considero que estos conocimientos me servirán bastante y son una herramienta valiosa que quiero seguir explorando, definitivamente.
    """
    ...
    st.markdown("<h1 style='text-align: center;'>Los primeros gráficos</h1>", unsafe_allow_html=True)

    # Creamos una lista de gráficos
    graficos = ['Gráfico de nube de palabras', 'Gráfico de pastel sobre la distribución de películas y series en Netflix', 'Mapa de las pelis']
    ...
    if grafico_seleccionado == 'Gráfico de nube de palabras':
        st.markdown("<div style='text-align: justify; font-size: 20px;'>En la nube de palabras, teniendo en cuenta la cantidad de veces en que se repitan, se grafica la palabra de mayor o menor tamaño. En esta, luego de un proceso de filtración, se muestra que las palabras más recurrentes son “better”, “like”, “go”, “got”, entre otras.</div>", unsafe_allow_html=True)
        st.image("nube.png", caption='Gráfico de nube de palabras', width=500)
        pass
    elif grafico_seleccionado == 'Gráfico de pastel sobre la distribución de películas y series en Netflix':
        st.markdown("<div style='text-align: justify; font-size: 20px;'>En el gráfico, se separan las películas de las series que se encuentran en la plataforma Netflix para determinar su distribución. Frente al resultado obtenido, se concluye que existen más películas que series en la misma.</div>", unsafe_allow_html=True)
        st.image("pastel.png", caption='Gráfico de pastel sobre la distribución de películas y series en Netflix', width=500)
        pass
    elif grafico_seleccionado == 'Mapa de las pelis':
        st.markdown("<div style='text-align: justify; font-size: 20px;'>El mapa incluye la localización de grabación de mis 5 películas favoritas</div>", unsafe_allow_html=True)
